...
class STrack(BaseTrack):
    ...
    def multi_predict_diff(stracks, model, img_w, img_h):
        if len(stracks) > 0:
            ...
            model.eval()
            multi_track_pred = model.generate(conds, sample=1, bestof=True, img_w=img_w, img_h=img_h, can_work_with_arbitrary_length=False)
    ...
    def tlwh_buffer(self, factor=1.0):
        ret = self.tlwh.copy()
        ret[2:] *= factor
        return ret
